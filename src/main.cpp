...
void getRequest(CCLayer* self, GJGameLevel* level, CCLabelBMFont* thelabel, bool pointercrate, bool platformer)
{
    ...
    // demonlist.org and pemonlist use "placement"; challengelist uses "position"
    std::string positionstring = pointercrate ? "placement" : "position";
    if (platformer) positionstring = "placement";

    std::string url = pointercrate ? "https://api.demonlist.org/level/classic/get?ingame_id=" + std::to_string(lvlID) : "https://challengelist.gd/api/v2/demons/listed/?name=" + std::string(lvlname);
    if (platformer) url = "https://pemonlist.com/api/level/" + std::to_string(lvlID);
    // positionstring is "placement" for both demonlist.org (pointercrate path) and pemonlist (platformer path)
    // challengelist uses "position" which is handled via its own array parsing below
    ...
    webreq.spawn(
        ...
        [self = geode::Ref(self), thelabel = geode::Ref(thelabel), pointercrate, level = geode::Ref(level), platformer, positionstring, lvlID, onlyRemoveOnSuccess = geode::Ref(onlyRemoveOnSuccess)](geode::utils::web::WebResponse res) mutable {
            ...
            log::info("{}\n\n", resultat);
            // demonlist.org returns { data: {...} }, challengelist/pemonlist return array or object directly
            std::string result;
            if (!platformer && !pointercrate) {
                ...
                // pointercrate path now uses demonlist.org — parse as-is
                result = resultat;
            }
            ...
            if (pointercrate) {
                // demonlist.org wraps the level data under a "data" key
                if (!childJson.isObject() || !childJson.contains("data") || childJson["data"].size() == 0) {
                ...
                child = childJson["data"];
            } else if (childJson.isArray() && childJson.size() > 0) {
                child = childJson[0];
